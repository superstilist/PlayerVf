...
VideoOutput::~VideoOutput() {
  ...
  if (render_context != nullptr) {
    auto future = thread_pool_ref_->Post([render_context]() {
...
void VideoOutput::NotifyRender() {
  if (destroyed_ || render_context_ == nullptr) {
    return;
  }
...
void VideoOutput::Render() {
  ...
  if (texture_id_) {
    ...
    if (pixel_buffer_ != nullptr) {
      int64_t texture_id = texture_id_;
      int64_t width = 0;
      int64_t height = 0;
      {
        std::lock_guard<std::mutex> lock(textures_mutex_);
        auto it = pixel_buffer_textures_.find(texture_id);
...
void VideoOutput::SetTextureUpdateCallback(
  ...
  // During startup mpv can report 0x0 for |video-out-params| while the
  // bootstrap texture is already registered. Dart waits for this first texture
  // notification before allowing Player commands to continue, so report the
  // actual registered texture size until real video dimensions are known.
  if (width < 1 || height < 1) {
    if (surface_manager_ != nullptr) {
      width = surface_manager_->width();
      height = surface_manager_->height();
    } else if (pixel_buffer_ != nullptr) {
      std::lock_guard<std::mutex> lock(textures_mutex_);
