...
class _VideoSeekIntent extends Intent {
  ...
  const _VideoSeekIntent(this.seconds);
}
...
class _VideoPageState extends State<VideoPage> {
  ...
  void didChangeDependencies() {
    ...
    _syncPalette(musicService.currentMusic?.coverPath);
  ...
  String _extractYoutubeVideoId(Music? music) {
    if (music == null || music.genre != 'YouTube Music Video') return '';
    final id = music.id;
  ...
  bool _looksLikeLocalVideo(Music music) {
    ...
    final ext = p.extension(music.filePath).toLowerCase();
    return const {'.mp4', '.mkv', '.webm', '.mov', '.avi', '.m4v'}
        .contains(ext);
  }
          ...
          child: Actions(
            actions: <Type, Action<Intent>>{
              ...
              _VideoSeekIntent: CallbackAction<_VideoSeekIntent>(
                onInvoke: (intent) {
                  _seekBy(musicService, Duration(seconds: intent.seconds));
                  return null;
                },
              ),
        ...
        children: [
          if (coverPath.isNotEmpty)
            ...
            StableVideoSurface(
              ...
              surfaceKey:
                  _videoSurfaceIdentity(currentMusic, prefix: 'background'),
              fit: BoxFit.cover,
            )
          else
        ...
        pageBuilder: (context, animation, secondaryAnimation) {
          return FadeTransition(
            ...
            child: _FullscreenVideoPage(
              ...
              surfaceKey: _videoSurfaceIdentity(musicService.currentMusic,
                  prefix: 'full'),
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ...stream.qualities.map((quality) {
                    ...
                    final canStream = quality.hasAudio;
                    return ListTile(
...
class _FullscreenVideoPageState extends State<_FullscreenVideoPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Shortcuts(
      shortcuts: const <ShortcutActivator, Intent>{
        SingleActivator(LogicalKeyboardKey.space): _VideoTogglePlayIntent(),
        SingleActivator(LogicalKeyboardKey.arrowLeft): _VideoSeekIntent(-10),
        SingleActivator(LogicalKeyboardKey.arrowRight): _VideoSeekIntent(10),
          ...
          _VideoSeekIntent: CallbackAction<_VideoSeekIntent>(
            onInvoke: (intent) {
              final next = widget.musicService.position +
                  Duration(seconds: intent.seconds);
