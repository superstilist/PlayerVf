...
class _YoutubeMusicPageState extends State<YoutubeMusicPage>
  ...
  Future<void> _download(YoutubeMusicResult item) async {
    ...
    setState(() {
      ...
      _message = 'Downloading ${item.displayTitle}...';
    });
    ...
    try {
      ...
      final refreshedDownloadSet = choice.video || choice.subtitlesOnly
          ? await _loadDownloadedYoutubeSet(item)
          : null;

      if (!mounted) return;
      setState(() {
        _downloadedCache[key] = _mergeDownloadedSet(
          _downloadedCache[key],
          refreshedDownloadSet ??
              _DownloadedYoutubeSet(
                qualityHeights: choice.video
                    ? {
                        if (choice.qualityHeight != null) choice.qualityHeight!,
                        ...choice.qualityHeights,
                      }
                    : const <int>{},
                subtitleKeys: <String>{},
                audio: !choice.video && !choice.subtitlesOnly,
              ),
        );
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ListTile(
                          ...
                          subtitle: Text(item.displayTitle,
                            ...
                            ...qualities.map((quality) {
                              ...
                              final saved =
                                  downloadedSet?.hasQuality(quality.height) ??
                                      false;
                              final downloadOnly = !quality.streamable;
                              return ChoiceChip(
                                avatar: Icon(saved
  ...
  Future<void> _readAudioDownloadMarks(
    ...
    for (final item in items) {
      ...
      final displayTitle = _downloadMarkText(item.displayTitle);
      final displayArtist = _downloadMarkText(item.displayArtist);
      if (title.isEmpty) continue;
      final matched = mediaNames.any((name) {
        final titleMatches = name.contains(title) ||
            (displayTitle.isNotEmpty && name.contains(displayTitle));
        final artistMatches = artist.isEmpty ||
            name.contains(artist) ||
            (displayArtist.isNotEmpty && name.contains(displayArtist));
  ...
  Future<void> _stream(YoutubeMusicResult item) async {
    ...
    setState(() {
      ...
      _message = 'Opening ${item.displayTitle}...';
    });

    try {
      final wantsVideo = _isVideoIntent(item);
      final stream = await _service.stream(item, audioOnly: !wantsVideo);
      if (!mounted) return;
      if (stream.url.isEmpty) {
        setState(() => _message = 'Could not open this YouTube Music stream.');
        return;
      }
      final playsAsVideo = stream.isVideo || wantsVideo;
      ...
      final music = Music(
        ...
        genre: playsAsVideo ? 'YouTube Music Video' : 'YouTube Music',
        year: '',
      ...
      if (!mounted) return;
      final cacheMessage = stream.cacheStatus == 'background'
          ? ' Streaming now; caching video in background.'
          : stream.cacheStatus == 'complete'
              ? ' Playing from video cache.'
              : stream.cacheStatus == 'incomplete'
                  ? ' Streaming now; cache will resume later.'
                  : stream.cacheStatus == 'disabled'
                      ? ' Stream cache is off.'
                      : '';
      setState(() => _message = 'Playing ${stream.displayTitle}.$cacheMessage');
      await playFuture;
    } on UnsupportedError catch (e) {
      if (mounted) setState(() => _message = e.message);
    } on MissingPluginException {
  ...
  bool _isVideoIntent(YoutubeMusicResult item) {
    final type = item.resultType.toLowerCase().trim();
    if (type == 'video' || type == 'videos') return true;
    return _selectedFilter == 'videos' && item.videoId.trim().isNotEmpty;
  }
  ...
  Widget _buildResultTile(
    ...
    final savedLabel = _downloadedLabel(item, downloadedSet);
    final subtitle = [
      if (item.displayArtist.isNotEmpty) item.displayArtist,
      if (item.duration.isNotEmpty)
        normalizePlaybackDurationText(item.duration),
      item.resultType,
      if (isDownloaded) savedLabel,
    ].join('  -  ');
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        item.displayTitle,
                        maxLines: item.titleRomaji.isNotEmpty ? 2 : 1,
                      ...
                      Text(
                        subtitle,
                        maxLines: item.artistRomaji.isNotEmpty ? 2 : 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(
                            fontSize: 12.sp,
                            color:
  ...
  String _downloadedLabel(
    YoutubeMusicResult item,
    _DownloadedYoutubeSet? downloadedSet,
  ) {
    if (downloadedSet == null || !downloadedSet.hasAny) return '';
    if (item.resultType.toLowerCase() == 'video' &&
        downloadedSet.qualityHeights.isNotEmpty) {
      final heights = downloadedSet.qualityHeights.toList()..sort();
      return 'saved ${heights.map((height) => '${height}p').join(', ')}';
    }
    return 'saved';
  }
