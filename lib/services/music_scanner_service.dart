...
class MusicScannerService {
  ...
  static Future<Database> _initializeCacheDb() async {
    ...
    final documentsDirectory = await getPlayerVfDocumentsDirectory();
    ...
    _cacheDbOpening = openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
          'CREATE INDEX idx_last_modified ON file_cache(last_modified)',
        );
      },
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 4) {
          await _addColumnIfMissing(db, 'file_cache', 'year', 'TEXT');
        }
        if (oldVersion < 5) {
          await db.execute(
  ...
  static Future<bool> _isPlayervfYoutubeAudioWithRecoverableCover(
      File file) async {
    final extension = p.extension(file.path).toLowerCase();
    if (extension != '.mp3') return false;
    final sidecar =
        File('${p.withoutExtension(file.path)}.playervf.audio.json');
  ...
  static Future<bool> _isNonPrimaryPlayervfQuality(File file) async {
    ...
    try {
      ...
      final primaryPath = _firstExistingManifestQualityPath(qualities);
  ...
  static String _firstExistingManifestQualityPath(List qualities) {
  ...
  static Future<List<String>> _scanIOSMusicFolders(
      {Function(String)? onProgress}) async {
    final docDir = await getPlayerVfDocumentsDirectory();
  ...
  static Future<Map<String, dynamic>> _parseTagsIsolateEntry(String path) async {
    return await ID3Parser().parseTagsFromFile(path, runPlatformChannels: false);
  }

  static Future<bool> _hasSidecarOrManifest(File file) async {
    final manifest = await _findPlayervfManifest(file.path);
    if (manifest != null) return true;
    final sidecar = File('${p.withoutExtension(file.path)}.playervf.audio.json');
    return await sidecar.exists();
  }
  ...
  static Future<List<Music>> createMusicListFromPaths(List<String> paths,
    ...
    final appDocDir = await getPlayerVfDocumentsDirectory();
      ...
      try {
        ...
        if (cached != null) {
          ...
          if (cachedWithHint.coverPath.isNotEmpty) {
            ...
            if (!await _isPlayervfYoutubeAudioWithRecoverableCover(file)) {
              return cachedWithHint;
            }
          }
        }
        ...
        final isPureDartFormat = extension == '.mp3' ||
            extension == '.flac' ||
            extension == '.wav' ||
            await _hasSidecarOrManifest(file);

        final tags = await Future.any([
          isPureDartFormat
              ? compute(_parseTagsIsolateEntry, path)
              : parser.parseTagsFromFile(path, runPlatformChannels: true),
